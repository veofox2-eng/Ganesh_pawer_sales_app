...
import { Users, PhoneCall, MessageCircle, User, ArrowLeft, CheckCircle, Check, Trash2, XCircle, Search, Play, Pause, FileText, Paperclip, Download, X, ChevronDown, UserPlus, Lock, Eye, EyeOff, Share2 } from 'lucide-react';
...
export default function AdminEmployeesDashboard() {
  ...
  const [activeAdminTab, setActiveAdminTab] = useState<'approvals' | 'deleted'>('approvals');
  const [adminApprovals, setAdminApprovals] = useState<any[]>([]);
  const [deletedAudios, setDeletedAudios] = useState<any[]>([]);
  const [detailsLoading, setDetailsLoading] = useState(false);

  useEffect(() => {
    if (!selectedEmpId || !employees.length) return;
    ...
    async function fetchDetails() {
      ...
      const { data: approvalsData } = await supabase
        .from('interactions')
        .select('*')
        .eq('author', selectedEmp.username)
        .ilike('type', '%APPROVAL%')
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      const appAdmins = profiles?.filter((p: any) => {
        const r = p.role?.toLowerCase() || '';
        const n = (p.username || '').toLowerCase();
        return (r === 'admin') && !n.includes('super') && !n.includes('backend') && !n.includes('access') && !p.is_deleted;
      }) || [];
      ...
      const { data: clients } = await supabase
        .from('clients')
        .select('*')
        .in('user_id', appAdmins.map(p => p.id));
      ...
      const { data: allLeadProjects } = await supabase
        ...
        .in('created_by', appAdmins.map(p => p.id));
      ...
      // Fetch Other Call Records (not assigned to client)
      ...
      // Process Data
      const empData = appAdmins.map(emp => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Add Table
    const tableData = [
      ['Total Clients', selectedEmp.stats.total],
      ['Follow-up', selectedEmp.stats.followUp],
      ['Converted', selectedEmp.stats.converted],
      ['Lost', selectedEmp.stats.lost],
      ['Deleted', selectedEmp.stats.deleted],
      ['Total Calls', selectedEmp.stats.callCount],
      ['WhatsApp', selectedEmp.stats.whatsappCount],
      ['Notes Added', selectedEmp.stats.noteCount],
      ['Attachments', selectedEmp.stats.attachmentCount],
    ];
    ...
    if (whatsappTableData.length > 0) {
      ...
      doc.setFontSize(14);
      doc.setTextColor('#1f2937');
      doc.text(`WhatsApp By Employee (${selectedEmp.username || selectedEmp.feature_flags?.email || 'Pending'})`, 40, nextY);
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      ...
      setToastMessage({ title: 'Success', message: 'Employee created successfully!', type: 'success' });
  ...
  const handleConfirmShare = async () => {
    ...
    try {
      ...
      setToastMessage({ title: 'Success', message: 'Clients shared successfully!', type: 'success' });
      setSelectedClients([]);
      setIsSelectionMode(false);
      setShowShareModal(false);
      fetchData(true);
    } catch (e: any) {
      setToastMessage({ title: 'Error', message: 'Error sharing clients: ' + e.message, type: 'error' });
    } finally {
          ...
          <div ref={scrollRef} style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            ...
            {selectedClient ? (
                ...
                <div style={{ background: 'linear-gradient(145deg, var(--surface), rgba(255,255,255,0.01))', borderRadius: 24, border: '1px solid var(--border)', padding: '2.5rem' }}>
                  <h3 style={{ margin: '0 0 2rem 0', fontSize: '1.25rem', color: 'var(--text)', fontWeight: 700 }}>
                    {filterStatus === 'CALLS' ? 'Call Timeline' :
                     ...
                     filterStatus === 'ATTACHMENTS' ? 'Attachments Timeline' : 'Interaction Timeline'}
                  </h3>
                  {(() => {
              ...
              <div style={{ padding: '2.5rem' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
                      ...
                      <tbody>
                        {filteredClients.length === 0 ? (
                          <tr><td colSpan={7} style={{ padding: '2rem', textAlign: 'center', color: 'var(--muted)' }}>No clients found for this filter.</td></tr>
                ...
                <div style={{ background: 'linear-gradient(145deg, var(--surface), rgba(255,255,255,0.01))', borderRadius: 24, border: '1px solid var(--border)', padding: '2rem', boxShadow: '0 8px 30px rgba(0,0,0,0.03)', marginTop: '2rem' }}>
                  <h3 style={{ margin: '0 0 1.5rem 0', fontSize: '1.25rem', color: 'var(--text)', fontWeight: 700 }}>Admin Activities</h3>
                  ...
                  <div style={{ 
                    ...
                    {[{ id: 'approvals', label: 'Approvals Given', Icon: CheckCircle }, { id: 'deleted', label: 'Deleted Call Records', Icon: Trash2 }].map(tab => (
                      <button 
                        key={tab.id}
                        onClick={() => setActiveAdminTab(tab.id as 'approvals' | 'deleted')}
                        style={{ 
                          position: 'relative',
                          ...
                          fontSize: '0.85rem', 
                          fontWeight: 600,
                          color: activeAdminTab === tab.id ? 'var(--accent)' : 'var(--muted)',
                      ...
                      >
                        {activeAdminTab === tab.id && (
                          <motion.div
                            ...
                            style={{ 
                              position: 'absolute', inset: 0, background: 'var(--surface)', borderRadius: '12px', 
                  ...
                  <div style={{ marginTop: '1rem', minHeight: 200 }}>
                    {detailsLoading ? (
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 24, padding: '2.5rem', width: '90%', maxWidth: 500, maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', position: 'relative' }}>
            ...
            <div style={{ padding: '1.5rem', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
              <button 
                onClick={() => {}}
                className="btn-hover"
                style={{ padding: '0.65rem 1rem', background: 'var(--surface)', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 10, fontWeight: 600, fontSize: '0.85rem', display: 'none', alignItems: 'center', gap: 6, cursor: 'pointer' }}
              >
              ...
              <button 
                onClick={() => setShowShareModal(false)}
                className="btn-hover"
                style={{ padding: '0.75rem 1.5rem', background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: 12, fontWeight: 600, cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button 
                ...
                className="btn-hover"
                style={{ padding: '0.75rem 1.5rem', background: (!shareTargetEmployee || shareLoading) ? 'var(--surface)' : 'var(--accent)', color: (!shareTargetEmployee || shareLoading) ? 'var(--muted)' : '#fff', border: (!shareTargetEmployee || shareLoading) ? '1px solid var(--border)' : 'none', borderRadius: 12, fontWeight: 600, cursor: (!shareTargetEmployee || shareLoading) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
              >
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 24, padding: '2.5rem', width: '90%', maxWidth: 450, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '0 1rem' }}>
                  <User size={16} color="var(--muted)" />
                  <input type="text" placeholder="Sales Executive" value={addEmpData.industry_position} onChange={e => setAddEmpData({ ...addEmpData, industry_position: e.target.value })} style={{ flex: 1, border: 'none', background: 'transparent', padding: '0.75rem', color: 'var(--text)', outline: 'none' }} />
                </div>
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'var(--bg2)', borderRadius: 24, padding: '2rem', width: '100%', maxWidth: 700, maxHeight: '90vh', overflowY: 'auto', border: '1px solid var(--border)', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', position: 'relative' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>Activity log for {selectedEmp.username}</p>
            </div>
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}
          >
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: '2rem', width: '90%', maxWidth: 400, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
              <div style={{ width: 64, height: 64, borderRadius: '50%', background: toastMessage.type === 'success' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: toastMessage.type === 'success' ? '#10b981' : '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem', boxShadow: toastMessage.type === 'success' ? '0 0 20px rgba(16,185,129,0.2)' : '0 0 20px rgba(239,68,68,0.2)' }}>
                {toastMessage.type === 'success' ? <CheckCircle size={32} /> : <XCircle size={32} />}
              </div>
              ...
              <button
                ...
                className="btn-hover"
                style={{ width: '100%', padding: '0.75rem', background: toastMessage.type === 'success' ? '#10b981' : '#ef4444', color: '#fff', border: 'none', borderRadius: 12, fontWeight: 600, fontSize: '1rem', cursor: 'pointer' }}
              >
                OK
              </button>
            </div>
