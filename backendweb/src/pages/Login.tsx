...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithPassword({ email: 'backendadmin1@gmail.com', password });
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <p style={{ fontSize: '0.875rem', color: 'var(--muted)', transition: 'color 0.4s ease' }}>
            SalesFlow Analytics · Admin Portal
          </p>
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
              <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 600, color: 'var(--muted)', marginBottom: '0.5rem', letterSpacing: '0.07em', textTransform: 'uppercase', transition: 'color 0.4s ease' }}>Email</label>
              <input type="email" value="backendadmin1@gmail.com" readOnly
                style={{ width: '100%', padding: '0.75rem 1rem', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--muted)', fontSize: '0.875rem', outline: 'none', cursor: 'not-allowed', fontFamily: 'inherit', transition: 'all 0.4s ease' }} />
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.5 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)}
                  ...
                  style={{ width: '100%', padding: '0.75rem 3rem', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text)', fontSize: '0.875rem', outline: 'none', fontFamily: 'inherit', cursor: 'none', transition: 'border-color 0.2s, background 0.4s ease, color 0.4s ease' }}
                  onFocus={(e) => e.target.style.borderColor = 'var(--accent)'}
                  onBlur={(e) => e.target.style.borderColor = 'var(--border)'}
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--muted)', cursor: 'none', display: 'flex', alignItems: 'center', padding: 4 }}>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.85 }}
          ...
          Authorised personnel only · Fox Editz © 2025
        </motion.p>
      </motion.div>
    </div>
  );
